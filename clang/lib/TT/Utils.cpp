...
bool isFunctionAllocator(std::string FuncName) {
  return std::find(_TTOpts.AllocatorFunctions.begin(),
...
bool canWrite(const std::string &FilePath) {
  ...
  return filePathStartsWith(FilePath, _TTOpts.BaseDir);
}
...
class _TTFormatStringHandler
    : public analyze_format_string::FormatStringHandler {
  unsigned DataStartIdx;
  std::set<unsigned> &StringArgIndices;

public:
  _TTFormatStringHandler(unsigned DataStartIdx,
                         std::set<unsigned> &StringArgIndices)
...
// not a big deal: TT may just infer some checked pointer types incorrectly.
void getPrintfStringArgIndices(const CallExpr *CE, const FunctionDecl *Callee,
  ...
  for (const FormatAttr *Attr : Callee->specific_attrs<FormatAttr>()) {
    ...
    _TTFormatStringHandler Handler(DataStartIdx, StringArgIndices);
