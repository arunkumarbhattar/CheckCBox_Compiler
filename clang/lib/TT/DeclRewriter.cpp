...
//
    ...
    } else if (auto *FRB = dyn_cast<FunctionBodyReplacement>(N)) {
      rewriteFunctionBody(FRB);
    }
    else {
...
void DeclRewriter::rewriteFunctionBody(FunctionBodyReplacement *N) {
...
bool FunctionDeclBuilder::VisitFunctionDecl(FunctionDecl *FD) {
  ...
  // places.
  if (!_TTOpts.InferTypesForUndefs && !FDConstraint->hasBody())
    return true;
...
FunctionDeclBuilder::buildDeclVar(const FVComponentVariable *CV,
  ...
  if (ItypeSolution ||
      (CheckedSolution && _TTOpts.ItypesForExtern && !StaticFunc)) {
