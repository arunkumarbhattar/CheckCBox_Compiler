...
void ProgramInfo::printAggregateStats(const std::set<std::string> &F,
  ...
  for (auto &I : Variables) {
    ...
    if (F.count(FileName) ||
        FileName.find(_CheckMateOpts.BaseDir) != std::string::npos) {
      if (C->isForValidDecl()) {
        FoundVars.clear();
...
void ProgramInfo::printStats(const std::set<std::string> &F, raw_ostream &O,
  ...
  if (!OnlySummary && !JsonFormat) {
    O << "Sound handling of var args functions:" << _CheckMateOpts.HandleVARARGS
      << "\n";
  }
...
void ProgramInfo::linkFunction(FunctionVariableConstraint *FV) {
  ...
  // that parameters and returns are considered unchecked. When CheckMate is run with
  ...
  auto LinkComponent = [this, Reason](const FVComponentVariable *FVC) {
    ...
    if (!_CheckMateOpts.InferTypesForUndefs &&
...
void ProgramInfo::addVariable(clang::DeclaratorDecl *D,
  ...
  if (FunctionDecl *FD = dyn_cast<FunctionDecl>(D)) {
    ...
    for (unsigned I = 0; I < FD->getNumParams(); I++) {
      ...
      if (_CheckMateOpts.AllTypes && FuncName == "main" && FD->isGlobal() && I == 1) {
...
void ProgramInfo::ensureNtCorrect(const QualType &QT,
  ...
  if (_CheckMateOpts.AllTypes && !canBeNtArray(QT)) {
