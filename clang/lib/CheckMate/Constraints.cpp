...
#include "clang/CheckMate/ConstraintsGraph.h"
#include <iostream>
#include <set>
...
bool Constraints::addConstraint(Constraint *C) {
  ...
  // for wildness.
  // This is needed as CheckMate will currently only report one cause of wildness
...
bool Constraints::graphBasedSolve() {
  ...
  if (Res && _CheckMateOpts.AllTypes) {
    Env.doCheckedSolve(false);
    bool RegularSolve = !(_CheckMateOpts.OnlyGreatestSol || _CheckMateOpts.OnlyLeastSol);

    if (_CheckMateOpts.OnlyLeastSol) {
    ...
    } else if (_CheckMateOpts.OnlyGreatestSol) {
