...
static const char *HelpOverview =
    "TT: Automatically Generate Sandbox-Specific Instrumentation required to call "
...
found, TT prints a warning. If one is found, TT looks up each source file
specified on the command line in the compilation database to find the compiler
options to use for that file. Thus, if you give TT a compilation database
...
-extra-arg and -extra-arg-before. If you want to run TT on all source files in
your compilation database, you must pass them on the command line; TT will not
take the list automatically from the compilation database. If you specify a
source file that is not in the database, TT will use the compiler options from
...
TT writes the new versions of your files. With either of these options, if TT
...
mode"), but TT still solves for changes to all files under the -base-dir that
...
static cl::opt<std::string> OptWildPtrInfoJson(
    ...
    cl::init("WildPtrStats.json"), cl::cat(_TTCategory));
...
static cl::opt<std::string> OptPerPtrWILDInfoJson(
    ...
    cl::init("PerWildPtrStats.json"), cl::cat(_TTCategory));
...
//  CcOptions.DebugArrSolver = DebugArrSolver;
  ...
  if (!_TTInterfacePtr) {
    // _TTInterface::create has already printed an error message. Just exit.
    return 1;
  }
  // See clang/docs/checkedc/TT/clang-tidy.md#_TT-name-prefix
  // NOLINTNEXTLINE(readability-identifier-naming)
  _TTInterface &_TTInterface = *_TTInterfacePtr;

  //if (OptVerbose)
    errs() << "Parsing source files.\n";

  // Build AST from source.
  if (!_TTInterface.parseASTs()) {
