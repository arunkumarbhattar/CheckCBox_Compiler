...
#ifndef LLVM_CLANG_CheckMate_AVARBOUNDSINFO_H
#define LLVM_CLANG_CheckMate_AVARBOUNDSINFO_H

#include "clang/CheckMate/ABounds.h"
#include "clang/CheckMate/AVarGraph.h"
#include "clang/CheckMate/ConstraintVariables.h"
#include "clang/CheckMate/CtxSensAVarBounds.h"
...
public:
  ...
  // be due to CheckMate rewriting limitations (assignments appearing inside macros),
...
private:
  ...
  // Check if a fresh lower bound can be be inserted by CheckMate for the pointer
  // corresponding to the bounds key. When a pointer needs a fresh lower bound,
  // it is possible that CheckMate will not support inserting the new declaration.
  // No array bounds can be inferred for such pointers.
  bool isEligibleForFreshLowerBound(BoundsKey BK);
};

#endif // LLVM_CLANG_CheckMate_AVARBOUNDSINFO_H
