...
#ifndef LLVM_CLANG_CheckMate_MULTIDECLS_H
#define LLVM_CLANG_CheckMate_MULTIDECLS_H
...
// Some more information about multi-decls in the context of CheckMate:
//
...
// tricky for CheckMate to rewrite. If the type of one member becomes a _Ptr (or
...
//
// To keep the logic simpler, if CheckMate needs to change the type of at least one
...
// TagDecl itself (not a pointer to it, etc.), then CheckMate uses that name for the
...
// which CheckMate might need to insert a reference to the unnamed TagDecl elsewhere
// even if the multi-decl isn't being rewritten. In these cases, CheckMate typically
...
private:
  ...
  // handle the TagDecl consistently when CheckMate naively rewrites the same header
...
#endif // LLVM_CLANG_CheckMate_MULTIDECLS_H
